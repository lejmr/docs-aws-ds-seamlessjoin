...
AD is a Microsoft technology allowing to manage resources inside a company. Theoretically, a company of any size longing from small bussines to international enterprise. There are plenty of good sources dealing with AD specific topics from the basics to very deep dive topics. For the purpose of this writing a reader can simple look at the AD as piece of server side software where computers can be registered to and user accounts can be centrally managed. Having such a setup, users can be granted rights to login to certain servers and so on. Subsequently, such an user account can login using one password that is always the same, and managed and known by the user only, no matter which server is logging to. Additionally, single-sign on (SSO) technique can be used, so user provides his/her credentials just once upon a time, and a special authenticating token is used for login onto server instead of credentials.

Basically, such a configuration allows removing of all public ssh keys from the use hence forming one of the [Zero trust](https://en.wikipedia.org/wiki/Zero_Trust_Networks) pilars.

Due to the AD origin, created by Microsoft around year 2000 which is when Microsoft literally hated Linux, the server registration process AKA joining used to be a bit clumsy. Historically, the joining process used to be a very manuall process requiring configuration of LDAP, Kerberos, some Samba components, and DNS, so the Linux's authentication layer could work with AD. On top of that, this layer could be slightly different depending on the used Linux distribution. In other words, Linux joining process wasnt really a piece of cake. Luckilly, with the advance of technologies such as [RealmD](https://www.freedesktop.org/software/realmd/) and [SystemD](https://www.freedesktop.org/wiki/Software/systemd/) that are de-facto standards for all modern distributions the joining process has become rather straighforward. Nevertheles, an administrator needs to login onto server and run through a few steps prior the server can be considered joined to an AD domain.

On the same page, forming an AD installation requires some knowledge. Here comes Amazon AWS with [AWS Directory Service](https://aws.amazon.com/directoryservice/) which delivers AD as a Service. AWS delivers several flavours of AD installations, but all flavors deliver fully functional AD installation within matter of minutes after installation. The AD can be installed through AWS web console, AWS Cloudformation, or [Terraform](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/directory_service_directory). The advantage of AD delivered by AWS is its integration allowing SSO, or server joining through *Launch instance* form. 

![Launch instance - join form](files/managed-AD-v2-05-2.png)

This joining method is a great acceleration for server joining to AD. This functionality was only usable by Windows server, and recently has been enabled for GNU\\Linux distributions with AWS SSM agent VERSION (TODO) installed. No suprise, this embedded joining process uses AWS System Manager (SSM) Run Document module on the background. Specifically, AWS SSM provides the managed document *AWS-JoinDirectoryServiceDomain* which can be easily used. The example of a Windows server joining is attached in *[ec2-management.tf](ec2-management.tf)*.

Bear in mind that AWS SSM documents are only triggered by creating an document *attachement* which can be a manual operation or is added by a 3rd party component such as *Launch instance* wizard. Subsequently, it means there is no AWS SSM document that would be removing servers from AD once they are deleted. Technically, it is not even possible without direct interaction of the operating system which would provide its deregistration. Impossibility of computer deregistration is a very significant disadvantage because cloud is a subject of change, and servers can be started and deleted many times a day. In other words, the basic integration provided by AWS is great for an Cloud deployments which are mimicing on-premise deployments with semi-permanent servers. However, stale computer object can be pestering admins by removing plenty of servers either manually or using some periodically triggered scripts.
